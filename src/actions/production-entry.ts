...
import { reconcileOrderMaterials, reconcileMaterialsForStage } from "./material-consumption";
...
export async function approveProductionEntry(
    ...
    try {
        ...
        if (nextStage === ProductionStage.COMPLETED) {
            await reconcileOrderMaterials(entry.orderId);
        } else {
            // Reconcile unused materials allocated for the completed stage
            await reconcileMaterialsForStage(entry.orderId, entry.stage, session.user.id!);
        }
